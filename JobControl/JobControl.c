...
void updateJobList(job **job_list, int remove_done_jobs){
	...
	while(current_job!=NULL){
            insideJob=current_job;
            while (insideJob!=NULL)
            {
                if(remove_done_jobs==1&&current_job->status==DONE){
                    printf("%d", current_job->idx);
                    printf("     %s",statusToStr(current_job->status));
                    printf("         %s\n",current_job -> cmd);
			removeJob(job_list,current_job);
                }
                
                else{
                        if(insideJob->status==DONE){
                            done=1;
                        }
			else{
                                done=0;
                                if(done==0){
				pid_t processId;
				processId=waitpid(WAIT_ANY,&(insideJob->status),WNOHANG);
				if(processId>0){
					insideJob->status=DONE;
                                        done=1;
				}
                            }
			}
...
void runJobInForeground (job** job_list, job *j, int cont, struct termios* shell_tmodes, pid_t shell_pgid){
     ...
     int stat;
     int done=0;
     if(waitpid(j->pgid,&stat, WNOHANG)==-1){
        j->status=-1;
        printf("%d", j->idx);
        printf("     %s",statusToStr(j->status));
        printf("         %s\n",j -> cmd);
    ...
    if((done==0)&&(j->status==0||j->status==1)){
